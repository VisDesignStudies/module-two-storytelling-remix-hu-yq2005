(() => {
  const EVENT_NAME = "kcr:year";

  const fmtComma = d3.format(",");
  const accent = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#6a3e2f";
  const muted = getComputedStyle(document.documentElement).getPropertyValue("--muted").trim() || "#6f635d";

  function publishYear(year) {
    window.dispatchEvent(new CustomEvent(EVENT_NAME, { detail: { year } }));
  }

  function renderShopLine(data) {
    const svg = d3.select("#chart-shops");
    const tooltip = d3.select("#tooltip1");

    const width = 900;
    const height = 360;
    const margin = { top: 20, right: 25, bottom: 50, left: 70 };
    const innerW = width - margin.left - margin.right;
    const innerH = height - margin.top - margin.bottom;

    const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);

    const x = d3
      .scaleLinear()
      .domain(d3.extent(data, (d) => d.year))
      .range([0, innerW]);

    const y = d3
      .scaleLinear()
      .domain([0, d3.max(data, (d) => d.coffee_shop_count) * 1.08])
      .nice()
      .range([innerH, 0]);

    g.append("g")
      .attr("class", "axis")
      .attr("transform", `translate(0,${innerH})`)
      .call(d3.axisBottom(x).tickFormat(d3.format("d")));

    g.append("g")
      .attr("class", "axis")
      .call(d3.axisLeft(y).ticks(6).tickFormat((d) => fmtComma(d)));

    g.append("text")
      .attr("x", -innerH / 2)
      .attr("y", -50)
      .attr("transform", "rotate(-90)")
      .attr("fill", muted)
      .attr("text-anchor", "middle")
      .text("Number of coffee shops");

    const line = d3
      .line()
      .x((d) => x(d.year))
      .y((d) => y(d.coffee_shop_count));

    g.append("path")
      .datum(data)
      .attr("fill", "none")
      .attr("stroke", accent)
      .attr("stroke-width", 3)
      .attr("d", line);

    const points = g
      .selectAll("circle")
      .data(data)
      .join("circle")
      .attr("class", "shops-year-point")
      .attr("cx", (d) => x(d.year))
      .attr("cy", (d) => y(d.coffee_shop_count))
      .attr("r", 5)
      .attr("fill", "#fffdf9")
      .attr("stroke", accent)
      .attr("stroke-width", 2)
      .on("mousemove", function (event, d) {
        publishYear(d.year);
        tooltip
          .style("opacity", 1)
          .style("left", `${event.offsetX + 18}px`)
          .style("top", `${event.offsetY}px`)
          .html(`<strong>${d.year}</strong><br>${fmtComma(d.coffee_shop_count)} coffee shops`);
      })
      .on("mouseleave", function () {
        publishYear(null);
        tooltip.style("opacity", 0);
      });

    const labels = g
      .selectAll(".shops-year-label")
      .data(data)
      .join("text")
      .attr("class", "shops-year-label")
      .attr("x", (d) => x(d.year))
      .attr("y", (d) => y(d.coffee_shop_count) - 12)
      .attr("text-anchor", "middle")
      .attr("font-size", 11)
      .attr("fill", muted)
      .text((d) => d.year);

    function applyHighlight(year) {
      points
        .attr("r", (d) => (d.year === year ? 7 : 5))
        .attr("stroke-width", (d) => (d.year === year ? 3 : 2));

      labels
        .attr("font-weight", (d) => (d.year === year ? 700 : 400))
        .attr("fill", (d) => (d.year === year ? accent : muted));
    }

    window.addEventListener(EVENT_NAME, (e) => {
      applyHighlight(e.detail?.year ?? null);
    });
  }

  d3.csv("data/coffee_shop_counts.csv", d3.autoType).then((shops) => {
    renderShopLine(shops);
  });
})();
